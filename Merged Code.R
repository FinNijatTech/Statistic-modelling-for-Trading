...
for (selected_quarter in c("2021_Q1", "2021_Q3", "2021_Q4", 
  ...
  data.group1$positioSP.mom[times(times_) <= times("09:55:00") | 
                              times(times_) > times("15:40:00")] <- 0
  
  data.group1$positionNQ.mom[times(times_) <= times("09:55:00") | 
                               times(times_) > times("15:40:00")] <- 0

  data.group1$positionSP.mr[times(times_) <= times("09:55:00") | 
                                times(times_) > times("15:40:00")] <- 0
  
  data.group1$positionNQ.mr[times(times_) <= times("09:55:00") | 
                              times(times_) > times("15:40:00")] <- 0
  ...
  data.group1$ntransNQ.mr * 10
  
  data.group1$pnl_netSP.mr <- data.group1$pnl_grossSP.mr  -
  data.group1$ntransSP.mr * 10
  ...
  av.daily.ntrades = mean(data.group1.daily$ntransSP.mom + 
                            data.group1.daily$ntransNQ.mom, na.rm = TRUE) + mean(data.group1.daily$ntransSP.mr + 
                              data.group1.daily$ntransNQ.mr, na.rm = TRUE)
  # P&L
  grossPnL = sum(data.group1.daily$pnl_gross.mom) + sum(data.group1.daily$pln_gross.mr)
  netPnL = sum(data.group1.daily$pnl_net.mom) + sum(data.group1.daily$pnl_net.mr)
  
  # stat (eyni)
  stat = netCR * max(0, log(abs(netPnL/1000)))
  
  # collecting all statistics for a particular quarter (eyni)
  ...
  # collect summaries for all quarters (eyni)
  ...
  # create a plot of Gross and Net P&L and save it to png file (eyni)
